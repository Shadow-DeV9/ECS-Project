...
GitHub generates a short-lived OIDC token for each workflow run
AWS validates the token against the GitHub OIDC provider
GitHub Actions assumes the github-actions-oidc-role IAM role
The role has only the permissions it needs (ECR, ECS, S3, Terraform)
yaml
Copy
